...
pub fn build(b: *std.Build) !void {
    ...
    hal.addIncludePath(b.path("Inc"));

    hal.installHeadersDirectory(b.path("Inc"), "", .{});

    const cmsis_5 = b.dependency("cmsis_5", .{
        .target = target,
        .optimize = optimize,
    }).artifact("cmsis_5");
    hal.installLibraryHeaders(cmsis_5);
    hal.linkLibrary(cmsis_5);

    const cmsis_device_h7rs = b.dependency("cmsis_device_h7rs", .{
        .target = target,
        .optimize = optimize,
    }).artifact("cmsis_device_h7rs");
    hal.installLibraryHeaders(cmsis_device_h7rs);
    hal.linkLibrary(cmsis_device_h7rs);
